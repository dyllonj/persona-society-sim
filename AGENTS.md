...
Agents derive trait alphas from their `PersonaCoeffs`, clamp them via `SafetyGovernor`, and hand them to `SteeringController` so the residual additions happen in the specified decoder layers (`agents/language_backend.py`, `steering/hooks.py`). We now log both the baseline coefficients and the per-message `steering_snapshot` alphas so downstream tooling can correlate behavior with either static persona design or real-time adjustments (e.g., safety backoff events).
