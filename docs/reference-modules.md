...
- **`ObjectiveManager`** keeps every agent perpetually assigned one active `Objective` (auto-reassigning on completion/failure), tracking progress by scanning successful `ActionLog`s against the objective's `requirements` dict. Two requirement keys (`fill_field`, `scan`) only count progress when `action_log.info` carries the exact **string** `"1"` — not a boolean or int — a strict-equality trap if the action layer's output shape ever changes.
- **`ProbeManager`** schedules at most one self-report/behavioral probe per agent at a time, on a per-kind cooldown. See [explanation-known-gaps.md](explanation-known-gaps.md#probe-preamble-is-injected-twice) for a duplication bug in how probe text gets injected.
